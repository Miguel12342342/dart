...
├── funcoes/             # Funções e callbacks
├── condicionais/        # if, switch e desafios
├── colecoes/            # List, Map e Set
└── lacos_repeticoes/    # for, while e variações
```
...
| `booleans.dart` | Booleanos | Tipo `bool`, operadores relacionais, `&&`, `if/else` |
...
| `funcoes_part2.dart` | Tipos de parâmetros | Posicionais, nomeados (`{}`), obrigatórios (`required`), com valor padrão |
| `funcoes_part3.dart` | Escopo e anônimas | Funções globais, de escopo privado e anônimas |
...
| `funcoes_part5.dart` | Callbacks | `List<void Function()>`, simulação de `onPressed` |
...
| `switch.dart` | switch / case | Alternativa ao if/else, `default`, `case` com `Random()` |
| `desafio_condicionais_1.dart` | Desafio 1 | `stdin`, `int.tryParse()`, classificação por faixa etária |
...
| `sets.dart` | Set | Sem duplicatas, `add()`, `remove()`, `contains()`, `toSet().toList()` |
...
Substitua o caminho pelo arquivo que quiser executar.
